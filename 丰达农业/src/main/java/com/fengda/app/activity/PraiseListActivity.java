...
import com.fengda.app.adapter.PraiseListAdapter;
...
public class PraiseListActivity extends BaseActivity implements PullLayout.OnRefreshListener {
    ...
    protected void initViews() {
        ...
        if (type == 1) {//待产果
            ...
            topNvgBar.setTitle("消费果明细");
        }else {//吉柚美
            topNvgBar.setTitle("吉柚美明细");
        }
    ...
    public void getExchangeList(final int state) {
        ...
        if (type == 1) {//待产果
            ...
            call = userBiz.consumptionLogList(token);
        }else {//吉柚美
            call = userBiz.JiYouMeiLogList(token);
        }
